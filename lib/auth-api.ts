  ...
  try {
    ...
    const { data, error } = await supabase.auth.signInWithOtp({
      ...
      options: {
        ...
        // emailRedirectTo를 제거하면 OTP 코드가 전송됨
      },
    })
    ...
    if (error) {
      ...
      return { success: false, error: "인증 코드 전송에 실패했습니다. 잠시 후 다시 시도해주세요." }
    }

    // 성공 시 로그 (개발 환경에서 디버깅용)
    if (process.env.NODE_ENV === "development") {
      console.log("OTP 전송 성공:", email)
      console.log("💡 이메일을 확인하고 6자리 코드를 입력해주세요.")
    }

    return { success: true }
  } catch (error: any) {
    console.error("OTP 전송 중 오류:", error)
    return { success: false, error: error?.message || "인증 코드 전송 중 오류가 발생했습니다." }
  }
}
